...
// char *argv[];
{
	...
	if(argc > 1){/*direct connect to server*/
		hostname = argv[1];
		
		if((sockfd = tcp_open(hostname, NULL, SERV_TCP_PORT)) > 0)
	...
	for(;;) {
		...
		if(strcmp(cmd, command[0]) == 0) {/*open*/
		...
		else if(strcmp(cmd, command[1]) == 0) {/*cd*/
			if(connection_established)
				chdir_req(param_no - 1); /*cd request to server*/
			else
				fprintf(stderr, "You should connect to server, first\n");
		}
		else if(strcmp(cmd, command[2]) == 0) {/*lcd*/
			lchdir(param_no - 1);
		}
		else if(strcmp(cmd, command[3]) == 0) {/*ls*/
			if(connection_established)
				ls_req(param_no - 1); /*ls request to server*/
			else
				fprintf(stderr, "You should connect to server, first\n");
		}
		else if(strcmp(cmd, command[4]) == 0) {/*local ls*/
			lls(param_no - 1);
		}
		else if(strcmp(cmd, command[5]) == 0) {/*get*/
			if(connection_established) {
				if(param_no < 2)
		...
		else if(strcmp(cmd, command[6]) == 0) {/*put*/
			if(connection_established) {
				if(param_no < 2)
					fprintf(stderr, "You should specify the file name\n");
				else
		...
		else if(strcmp(cmd, command[7]) == 0) {/*close*/
			if(connection_established) {
				phead.cmd = 'e';
		...
		else if(strcmp(cmd, command[8]) == 0) {/*quit*/
			if(connection_established) {
				phead.cmd = 'e';
				iov[0].iov_base = (char *)&phead;
				iov[0].iov_len = sizeof(header);
		...
		else if(strcmp(cmd, command[9]) == 0) {/*hash*/
			if(hash) {
				hash = 0;
