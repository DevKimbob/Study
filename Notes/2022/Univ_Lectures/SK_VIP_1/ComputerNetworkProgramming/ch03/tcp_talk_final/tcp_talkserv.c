...
int input_and_send(int sd) {
    ...
    // set signal handler for "User1" signal
...
int recv_and_print(int sd) {
    ...
    while(1) {
        ...
        if (strstr(buf, EXIT_STRING) != NULL) {
            ...
            if (ppid != 1)  // check if the current process is a Orphan process
                kill(ppid, SIGUSR1);  // send "User1" signal to the Parent
...
void sig_handler(int signo) {
    puts("From client : Good bye.");  // Inform text

    // close opponent's child process
    write(accp_sock, EXIT_STRING, sizeof(EXIT_STRING));
    close(accp_sock);  // close current(parent)
    exit(0);  // terminate current(parent) process
}